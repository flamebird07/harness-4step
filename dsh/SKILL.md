...
# 四步法 Harness（DeepSeek Harness 适配层 v13.0.43 — DSH subagent 为主 + CLI 可选；r4 CLI 障碍修复）
...
- **r4 CLI 障碍修复（opencode 层，dsh 仅同步版本与设计意图）**：`scripts/ps.sh` 执行层拦截（F-01/F-02）、claude step1/2 scoped allowedTools（F-04）、run_step 读 config timeout（F-05）、quota 排队等待内层重试环修复 P-06（F-06）、`INFRA_FAILURE_CRED_POOL_EXHAUSTED` 信号（F-10）、core-logic §4b 旧降级描述替换（F-11）。设计意图注释在 opencode/run_step.ps1 INFRA_RETRY 块留有 DSH/Hermes 扩展点。版本 13.0.42 → 13.0.43。
...
- **p20-prechunk-fix（backlog）**：共享规则随 opencode 层更新——prechunk 对所有绑定统一触发（含原生子代理绑定），99 handoff 聚合不再丢片（F-P20）；claude runner JSON parse 失败检测发 INFRA_FAILURE 信号（F-P18）；mimo runner `Text repetition detected` 签名 → `INFRA_FAILURE:text_repetition` + exit 13（F-P16）；新增 check-bom.ps1 + Step 0 fail-closed BOM gate（F-P17）；preflight 拒绝 bash 内联 PS 自动变量（F-P19）。DSH 侧 runner 结构与 opencode 平行，对应改动在下次 DSH runner 同步时落地。版本 13.0.39 → 13.0.40。
...
- 与共享核心版本对齐：OpenCode 的 Claude Windows 超时修复采用直接子进程、超时清理与按字符数预拆分；绑定仍由配置锁决定。

### v13.0.35 (2026-08-24)
- 与 Hermes / OpenCode 同步版本：OpenCode 超时拆分入口加固，不改变 DSH 原生调度。

### v13.0.34 (2026-08-24)
- 与 Hermes / OpenCode 同步版本：Codex Step 4 Windows 沙箱助手兼容修复，不改变 DSH 绑定。

### v13.0.33 (2026-08-24)
- 与 Hermes / OpenCode 同步版本：双端绑定改为配置声明式，Codex Step 4 强制只读沙箱。

### v13.0.32 (2026-08-24)

- **队列完整性对齐**：共享队列修复工具只恢复可验证的状态写回，不改变 DSH 的后端绑定或分派模型。

### v13.0.31 (2026-08-24)

- **绑定契约对齐**：后端和模型族必须由运行时锁配置声明；执行器不再固化每一步的 CLI 选择。

### v13.0.30 (2026-08-24)

- **双端契约同步**：与 Hermes 主层及 OpenCode 适配层统一版本；原生子代理不可用时保留本运行时错误，不得自动改调另一运行时或 CLI。
...
- **V10/V11 自修复**：`bash --timeout 300000 + Tee-Object` 实时透传（F-P06/F-P15），`dsh/scripts/run_step.ps1` 补 `MaxSplitDepth=3`/`MaxAttempts=3`/`blocked_split_limit/EXIT_CODE=3` 拆分闭环（F-P02），CLI 调用与编排流程改“立即拆分不停下”（F-P06/F-P14）。版本号 13.0.23 → 13.0.24。

### v13.0.23 (2026-08-19)

- 三平台版本对齐至 13.0.23（opencode 侧违规7 修复：step4 支持 opencode-sub/harness-verifier + mimo runner 根因修复 + UTF-8 stdin 字节直写）。版本号 13.0.22 → 13.0.23。
