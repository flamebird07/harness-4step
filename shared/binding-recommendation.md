...
> **仅推荐，不强制**。实际绑定以各平台绑定配置为准（Hermes: `~/.hermes/binding-lock.json`；opencode: 编排时选择）。
...
| Step 4 复审 | **必须与 Step3 不同模型族**、挑盲区 | codex | gemini / claude | 跨模型族才能抓出执行者的惯性盲区 |

## 组合示例（复制到各自绑定配置）

```
省钱组合:  Step1=claude  Step2=claude     Step3=mimo            Step4=codex
加强组合:  Step1=claude  Step2=kimi-plan  Step3=opencode-sub    Step4=gemini
...
- opencode 内如配了多模型，可给 `harness-auditor/verifier` 配更强 model、给 `harness-implementer` 配更快 model，等效于跨模型族