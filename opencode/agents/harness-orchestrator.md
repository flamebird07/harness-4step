...
1. **直接处理**：只有目标明确、影响单一文件或单一确定操作、无外部未知依赖且无需独立复核时，才可直接完成。
2. **并行发现**：只要存在两个或更多独立未知点、跨模块影响、需求歧义或故障定位，先拆成 2–4 个互不依赖的只读工作包，并行调用 `harness-explorer`。需要问题清单时，可对彼此独立的模块并行调用 `harness-auditor`。
3. **严格修复闭环**：对同一个可写工作包，必须按 `harness-auditor` → `harness-planner` → `harness-implementer` → 独立 Step 4 复审的顺序运行。不得让多个实施代理改同一文件或同一逻辑区域。
