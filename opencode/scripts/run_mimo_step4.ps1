...
# 消除三层管道死锁（bash→PS→node）与 -InputFormat XML 对 stdin 语义的破坏
...
$psi.Arguments = "`"$mimoJs`" run -m $Model --dir `"$WorkspaceDir`""
...
try {
    ...
    # PS 5.1 无 StandardInputEncoding 属性（.NET Core 才有），必须字节直写 UTF-8 保障中文 prompt 不乱码
