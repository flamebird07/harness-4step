...
- OpenCode 的 Claude runner 直接启动 `claude.exe` 并异步读写标准流，移除 Windows 上会遗留 stdin/子进程的 `Start-Job + 管道` 组合；超时会终止完整进程树。
- 预拆分同时按 UTF-8 字符数（6,000）触发，避免“少于 60 行但单行超长”的提示词绕过拆分而卡住 Claude CLI。

### v13.0.35 (2026-08-24)
- **OpenCode 超时闭环**：实际工作区编排器强制经 `run_step.ps1` 启动四步法、外层至少 300 秒并记录完整日志；保留 `EXIT_CODE=-2` 给脚本的预拆分/递归拆分逻辑，禁止外层 240 秒截断后跳过拆分。
...
- **复审安全修复**：Hermes `run_cli.py` 对 Codex Step 4 强制 `--sandbox read-only`，忽略任何更宽松配置；允许仅非写入式文件检查，禁止测试、构建、安装及所有写操作。

### v13.0.32 (2026-08-24)

- **队列修复工具**：新增 `scripts/repair_queue_metadata.py`，只修复可由现有证据确定的缺失 `task_id` 与“Step 3 成功但状态未回写”问题；不删除、取消、重跑或自行解除历史任务。
...
- **双端对称修复**：OpenCode 的 `opencode-sub` 从仅 Step 4 扩展为 Step 1–4 均可由锁显式绑定；`run_step.ps1` 以 `STEP` 与 `SUBAGENT` 信号移交对应原生角色。Hermes `run_cli()` 也强制要求显式授权才能覆盖绑定；两端原生能力失败均不得自动切 Hermes、OpenCode 或其他 CLI。版本检查同步核对 Git HEAD。
